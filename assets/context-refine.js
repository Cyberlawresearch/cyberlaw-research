...
(function (global) {
  ...
  // A topic is only a first-stage filter. Sharing one topic never by itself
  // establishes either a related-reading link or a timeline relationship.
  const TOPICS = [
    ['AI民事纠纷与司法救济', [['人工智能','AI','算法'], ['纠纷','民事','损害赔偿','侵权责任','禁令']]],
    ['智能体治理', [['智能体','agentic','ai agent','AI代理'], ['越权','权限','接管','入侵','授权','网络风险','非预期行动','工具调用']]],
    ['智能体治理', [['智能体互联网','数据智能体','代理协议','AIP','UTP','智能体互联','互操作']]],
    ['AI安全事件与披露义务', [['AI','人工智能','模型','OpenAI','智能体'], ['事故披露','事件报告','报告标准','失控','异常行为','失配','misalignment']]],
    ['前沿AI能力评测与安全防护', [['AI','人工智能','模型','OpenAI','智能体'], ['前沿能力','红队','安全评测','网络能力','Preparedness Framework','安全手册']]],
    ['生成式AI备案与服务管理', [['生成式','大模型'], ['备案','登记','服务管理','服务登记']]],
    ['公共部门AI与自动化行政', [['AI','人工智能','算法','自动化'], ['行政','政务','公共部门','监管执法','政府决策']]],
    ['诉讼与仲裁中的AI使用', [['AI','人工智能'], ['诉讼程序','证据使用','AI证据','材料真实性','使用披露','仲裁','法院提交']]],
    ['算法招聘与劳动管理', [['算法','AI','人工智能','自动化'], ['招聘','求职','用工','劳动','就业']]],
    ...
    ['公共数据开放与授权运营', [['公共数据','数据开放','授权运营','开放数据'], ['数据','授权','专区','开放']]],
    ...
    ['未成年人上网与年龄核验', [['儿童','未成年','青少年','children','13岁','15岁'], ['上网','社交','平台','核验','账户','聊天机器人','最低年龄']]],
    ['平台内容治理与DSA实施', [['DSA','数字服务法','内容治理','非法内容','推荐算法','平台问责']]],
    ['AI训练版权与数据许可', [['AI','人工智能','模型','OpenAI','Anthropic','TDM','Copilot','Codex'], ['版权','著作权','创作者作品','合理使用','fair use','开源许可','DMCA'], ['训练','training','挖掘','TDM','许可','输出']]],
    ['AI生成内容与作者资格', [['AI','人工智能','生成式'], ['作者资格','法定作者','可版权性','作品登记','著作权登记']]],
    ['深度合成侵权与内容标识', [['深度伪造','deepfake','换脸','拟声','AI脱衣','合成内容','数字水印'], ['侵权','禁令','标识','披露','规则','治理','禁止']]],
    ['平台反垄断与竞争执法', [['平台','Google','Apple','Microsoft','Amazon','Oracle','Meta','DMA','应用商店'], ['反垄断','竞争','垄断','自我优待','守门人','antitrust','拆分','并购']]],
    ['行政性市场壁垒与公平竞争', [['统一市场','行政垄断','歧视性准入','地方保护','公平竞争审查','垄断协议个人责任']]],
    ['个人信息保护处罚', [['个人信息','隐私','GDPR','数据保护'], ['处罚','罚款','行政处罚','fine','penalty','sanction']]],
    ...
    ['先进芯片与出口管制', [['芯片','半导体','GPU','先进计算','Ascend'], ['出口管制','出口限制','禁运','许可限制','实体清单']]],
    ...
    ['数据中心电力与资源监管', [['数据中心','算力基地'], ['能源','电力','电网','环境','空气许可','能耗','并网','水资源','电价','成本']]],
  ...
  function markerSet(item) {
    ...
    for (const re of [
      /《([^》]{3,80})》/g,
      /\b(?:EU KIDS Act|AI Act|GDPR|DSA|DMA|DMCA|Redata|Preparedness Framework|H\.R\.\s*\d+|GB\/[A-Z]?\s*\d+-\d{4}|Doe v\. GitHub)\b/gi
    ]) {
      let m;
      while ((m = re.exec(text))) {
        const value = normal(m[1] || m[0]);
  ...
  function markerOverlap(a, b) {
    const A = markerSet(a), B = markerSet(b);
    return [...A].filter(x => B.has(x)).length;
  }
  function sharedDetectedTopics(a, b) {
    const A = new Set(detectTopics(a).map(x => x.name));
    return detectTopics(b).map(x => x.name).filter(x => A.has(x));
  }
  function relationSignals(a, b) {
    return {
      entities: entityOverlap(a, b),
      markers: markerOverlap(a, b),
      title: similarity(a.title, b.title),
      facts: similarity(a.facts || '', b.facts || ''),
      topics: sharedDetectedTopics(a, b)
    };
  }
  function directlyRelated(a, b) {
    const s = relationSignals(a, b);
    if (s.markers > 0) return true;
    if (s.entities > 0 && s.topics.length > 0) return true;
    if (s.title >= .22) return true;
    if (s.topics.length > 0 && (s.title >= .10 || s.facts >= .12)) return true;
    return false;
  }
  function timelineRelation(a, b, topic) {
    const s = relationSignals(a, b);
  ...
  function buildTimelines(current, rows) {
    ...
    for (const topic of own) {
      const candidates = news.map(x => ({ x, rel: timelineRelation(current, x, topic.name) }))
  ...
  function relatedRows(current, rows, kind) {
    ...
    const ranked = candidates.map(x => {
      if (!directlyRelated(current, x)) return null;
      const s = relationSignals(current, x);
      const indexed = new Set(current.topics || []);
      const indexedShared = (x.topics || []).filter(t => indexed.has(t)).length;
  ...
  function renderRelated(panel, item, rows) {
    ...
    panel.innerHTML = `<div class="ctx-panel-title">关联阅读</div>${html || '<p class="ctx-empty">暂无直接相关内容。</p>'}`;
  }
  let serial = 0;
  function renderTimeline(panel, item, rows) {
    ...
    if (!groups.length) { panel.insertAdjacentHTML('beforeend','<p class="ctx-empty">暂无能够形成清晰发展脉络的专题。</p>'); return; }
